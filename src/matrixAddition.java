...
public class matrixAddition {
    ...
    static void add(int[][] a, int r1, int c1, int[][] b, int r2, int c2) {
        if (r1 != r2 || c1 != c2) {
            System.out.println("Matrix addition not possible - dimensions don't match");
            return;
        }

        int[][] sum = new int[r1][c1];
        for (int i = 0; i < r1; i++) {
            for (int j = 0; j < c1; j++) {  // Fixed: was c2, should be c1
                sum[i][j] = a[i][j] + b[i][j];
            }
        }
    ...
    public static void main(String[] args) {
        ...
        // Call the add method to perform addition and display result
        add(a, r1, c1, b, r2, c2);
